local g2 = require "sparky/sparky_g2"
...
function sparky.loop(g, memory)
    local user = g.items[g.you]

    function g2search(s)
        return g2.search(g, s)
    end

    local bot = memory[user.n]
    if not bot then
        bot = {}
        memory[user.n] = bot
        bot.first_turn = true
        bot.generate_tunnel_request = function(from, to, amount, user)
            local bot = memory[user.n]
            for i = 1, #from.closest_planets do
                local p = from.closest_planets[i]
        ...
        bot.future_ships = function(planet, time)
            ...
            if planet.ships_production_enabled then
                total = total + time * planet.ships_production * 0.02
            end
            return total
        end
        bot.amount_to_send = function(to, from)
            ...
            if to.ships_production_enabled then
                total = total + 0.00012 * to.ships_production * dist
            end
            return total
        end
        bot.is_worth_capturing_initially = function(neutral, time, home)
            return (-neutral.ships_value + (time - neutral:distance(home) * 0.025) * neutral.ships_production * 0.02) /
                (neutral.ships_value + 0.1) >
                0.3 and neutral.ships_production / (neutral.ships_value + 0.1) > 3
        end
        ...
        bot.planet_worth = function(to, from)
            return -to:distance(from) / 11 - 2.2 * to.ships_value + to.ships_production * 0.2
        end
        bot.strategic_value = function(to, from, user)
            return -to.ships_value + to.ships_production * (memory[user.n].horizon - to:distance(from) * 0.025)
        end
        bot.send_exact = function(user, from, to, ships)
            if from.ships_value < ships then
                local v = from.ships_value
    ...
    if bot.first_turn then
        ...
        if not bot.user_neutral then
            for _, u in pairs(g2search("user")) do
                if u.user_neutral then
                    bot.user_neutral = u
                    print(u)
                end
            end
        ...
        for i = 1, #planets do
            ...
            table.sort(
                planet_data.closest_planets,
                function(p1, p2)
                    if p1 and p2 then
                        return planet_data.dist[p1.n] < planet_data.dist[p2.n]
                    end
                end
            )
        end
        bot.target_planets = {}
        if not bot.enemy then
            local users = g2search("user -team:" .. user.team)
            for i = 1, #users do
                local u = users[i]
                if not u.user_neutral and u ~= user then
                    bot.enemy = u
                    break
                end
            end
        ...
        local user_planets = g2search("planet owner:" .. user.n)
        bot.home = user_planets[1]
        local enemy_planets = g2search("planet -neutral -team:" .. user.team)
        bot.enemy_home = enemy_planets[1]
        if not bot.enemy_home or not bot.home then
            return
        end
        ...
        for i, planet_data in pairs(bot.planets) do
            ...
            for j, planet_2 in pairs(planet_data.closest_planets) do
                ...
                if value > 0 then
                    planet_worth = planet_worth + value
                end
            end
        ...
        if source.planet.owner_n ~= bot.enemy.n then
            ...
            for j, planet in pairs(source.closest_planets) do
                if planet.owner_n == bot.enemy.n then
                    closest_enemy = bot.planets[planet.n]
                    break
                end
            end
    ...
    if bot.first_turn then
        ...
        for i, planet_data in pairs(front_planets) do
            send_exact(
                ...
                bot.home,
                planet_data.planet,
                bot.home.ships_value * planet_data.planet_worth / total_front_planet_worth
            )
        end
    end
    ...
    for i, target_data in pairs(bot.planets) do
        local fleets = g2search("fleet target:" .. target_data.planet)
        target_data.incoming_fleets = {}
        for i = 1, #fleets do
            local fleet = fleets[i]
            target_data.incoming_fleets[#target_data.incoming_fleets + 1] = {
                fleet = fleet,
                dist = fleet:distance(target_data.planet)
            }
            local sign = -1
            if target_data.planet.owner_n == fleet.owner_n then
                sign = 1
            end
            ...
            if target_data.planet.owner_n == user.n and sign == -1 then
                ...
                if closest_friendly ~= nil then
                    local ships_to_reserve =
                        fleet.fleet_ships -
                        (closest_distance * 0.025 - 3) * closest_friendly.planet.ships_production * 0.02 +
                        1
                    if ships_to_reserve < 0 then
                        ships_to_reserve = 0
                    end
                ...
                local ships_to_reserve =
                    fleet.fleet_ships -
                    (fleet:distance(target_data.planet) * 0.025 - 3) * target_data.planet.ships_production * 0.02 +
                    1
                if ships_to_reserve < 0 then
                    ships_to_reserve = 0
                end
            ...
            if planet_data.planet.ships_value > 6 then
                ...
                if fleets[1] ~= nil and fleets[2] ~= nil then
                    if fleets[1].fleet.owner_n == user.n and fleets[2].fleet.owner_n == bot.enemy.n then
                        ...
                        if
                            (-planet_data.planet.ships_value + fleets[1].fleet.fleet_ships) +
                                (arrival_time_2 - arrival_time_1) * planet_data.planet.ships_production * 0.02 <
                                fleets[2].fleet.fleet_ships or
                                (arrival_time_2 - arrival_time_1 - 2) * planet_data.planet.ships_production * 0.02 <
                                    planet_data.planet.ships_value
                         then
                            -- redirect to the closest enemy planet
                            local closest_enemy
                            for j, new_target in pairs(planet_data.closest_planets) do
                                if new_target.owner_n == bot.enemy.n then
                                    closest_enemy = bot.planets[new_target.n]
                                    break
                                end
                            end
                            if closest_enemy ~= nil then
    ...
    for i, source in pairs(bot.target_planets) do
        if
            source.planet.owner_n == user.n and
                (source.net_ships < 0 or (source.is_under_attack and source.net_ships < 2))
         then
            for k = 1, 4 do
                for j, helper in pairs(source.closest_planets) do
                    ...
                    if source.dist[helper.n] < 250 and source.net_ships < 0 and helper_data.net_ships > 0 then
                        if helper.owner_n == user.n then
                            ...
                            if amount_needed > helper_data.net_ships * 0.3333 + 1 then
                                amount_needed = helper_data.net_ships * 0.3333 + 1
                            end
            ...
            if target.owner_n == bot.enemy.n then
                ...
                if target_planet_data.net_ships > 0 or target_planet_data.net_ships + fleet.fleet_ships < 0 then
                    ...
                    if planet_to_attack ~= target_planet_data.planet then
                        ...
                        local sign = -1
                        if planet_to_attack.owner_n == fleet.fleet_target then
                            sign = 1
                        end
                    ...
                    if planet.owner_n == bot.enemy.n then
                        ...
                        if amount_to_send > source.net_ships then
                            ...
                            if amount_to_send > 0 then
                                local amount_sent =
                                    send_exact(user, source.planet, closest_enemy.planet, amount_to_send)
                                source.net_ships = source.net_ships - amount_sent
